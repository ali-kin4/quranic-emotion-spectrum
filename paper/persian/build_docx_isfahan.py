...
Pipeline:
  1. Preprocess manuscript.md â†’ strip LaTeX-only commands, convert \emph{}/\textbf{}
     to markdown italic/bold, drop xepersian-specific wrappers.
  2. Build a styled reference.docx (B Lotus 12pt body RTL, Amiri for Arabic,
     Times New Roman for Latin, proper heading hierarchy).
  3. Run pandoc with --reference-doc to convert .md â†’ .docx.
...
MD_IN = HERE / "manuscript_isfahan.md"
MD_TMP = HERE / "_manuscript_for_docx.md"
REF_DOCX = HERE / "reference_isfahan.docx"
OUT_DOCX = HERE / "manuscript_isfahan.docx"
...
def preprocess_markdown(text: str) -> str:
    ...
    # can apply LTR direction + Times New Roman font.
    def _strip_eng(m):
        body = m.group(1)
        # Convert \textbf{...} â†’ **...**, \textit{...} â†’ *...*
...
def build_reference_docx():
    ...
    # captured to base via shell redirect â€” re-extract just in case
    if not base.exists():
    ...
    def set_font(run_or_style, *, latin="Times New Roman", complex_script="B Lotus",
    ...
    # --- Body / Normal style: B Lotus 12pt, RTL, justified ---
    ...
    if bidi is None:
        bidi = OxmlElement('w:bidi')
        pPr.append(bidi)
    # Set complex-script (RTL) font to B Lotus at the style level
    ...
    if szCs is None:
        ...
        rPr.append(szCs)
    szCs.set(qn('w:val'), '26')  # 24 half-points = 12pt
    ...
    for style_name, size, bold in heading_specs:
        ...
        if rFonts is None:
            ...
            rPr.insert(0, rFonts)
        rFonts.set(qn('w:cs'), 'B Lotus')
        rFonts.set(qn('w:ascii'), 'B Lotus')
        rFonts.set(qn('w:hAnsi'), 'B Lotus')
        szCs = rPr.find(qn('w:szCs'))
        if szCs is None:
            szCs = OxmlElement('w:szCs')
            rPr.append(szCs)
...
def postprocess_docx():
    """Walk every paragraph, set RTL bidi + B Lotus complex-script font.
    ...
    def set_run_fonts(run, *, latin="Times New Roman", cs="B Lotus"):
    ...
    for para in doc.paragraphs:
        ...
        if in_english:
            ...
            for run in para.runs:
                set_run_fonts(run, latin="Times New Roman", cs="Times New Roman")
        else:
            set_rtl(para)
            if para.style.name == 'Normal':
                para.paragraph_format.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
            for run in para.runs:
                # B Lotus for Persian/Arabic; Amiri may be better for Arabic
                # verses but full-string detection is brittle; we rely on
                # B Lotus falling back to Amiri for missing glyphs.
                set_run_fonts(run, latin="Times New Roman", cs="B Lotus")

    # Remove marker paragraphs
    for p in paragraphs_to_remove:
        p._element.getparent().remove(p._element)

    # Process tables: set RTL on every cell paragraph + B Lotus font
    ...
    print(f"Post-processed {OUT_DOCX} â€” RTL applied, fonts set, markers removed.")
